...
export default function ACaminho() {
    ...
    async function carregar() {
        ...
        const resp = await fetch(`${API_URL}/processo/a-caminho`);
    ...
    async function confirmarRecebido(venda_numero) {
        await fetch(`${API_URL}/processo/chegou`, {
            method: "POST",
                    ...
                    {v.usuario.whatsapp && (
                        <a
                            ...
                            href={`https://wa.me/55${v.usuario.whatsapp}?text=${encodeURIComponent(
                                `Olá, tudo bem? Aqui é da Missionary Store Brasil. Gostaria de confirmar com vôce se recebeu seu pedido: ${v.produtos.map(p => p.nome).join(", ")}`
                            )}`}
                            target="_blank"
                            rel="noreferrer"
                        >
